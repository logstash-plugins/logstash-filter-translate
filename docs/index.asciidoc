...
Then, if logstash received an event with the field `data` set to `foo`, the destination
field would be set to `bar`. However, if logstash received an event with `data` set to `nope`,
